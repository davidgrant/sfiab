...
function get_winners($awardid, $fair_id)
{
 	global $config;
...
/*				'principal'=>'principal',
	...
	if($awardid == -1) {
		/* Get all for this fair */
	 	$q=mysql_query("SELECT * FROM award_awards WHERE award_source_fair_id='$fair_id' AND year='{$config['FAIRYEAR']}'");
	...
	$q = mysql_query("SELECT * FROM fairs WHERE id='{$award['award_source_fair_id']}'");
	...
	foreach($awards as $award) {
		...
		while($prize=mysql_fetch_assoc($q)) {
			...
			$wq=mysql_query("SELECT projects.* FROM award_prizes
						LEFT JOIN winners ON winners.award_prize_id=award_prizes.id
						LEFT JOIN projects ON projects.pid=winners.pid
					WHERE 
						award_prize_id='$pid' AND 
			...
			while($project=mysql_fetch_assoc($wq)) {
				...
				$prizewinners[]=array(	'xml_type' => 'project',/* for ysc compatability */
							...
							'title'=>utf8_encode($project['title']),
							'abstract'=>utf8_encode($project['summary']),
...
function count_winners($awardid, $fair_id)
{
	global $config;
	$count = 0;
	$awards = array();
...
function load_server_cats_divs($fair_id)
{
	global $config;
	$q = mysql_query("SELECT * FROM fairs WHERE id='$fair_id'");
	$fair = mysql_fetch_assoc($q);
...
case 'award_upload':
	...
	$fair_id = intval($_POST['fair_id']);
	$divs = $_POST['div'];
	$cats = $_POST['cat'];


	$all_winners = get_winners($award_awards_id, $fair_id);
...
case 'catdiv_load':
	$fair_id = intval($_GET['fair_id']);

	list($c, $d, $cm, $dm) = load_server_cats_divs($fair_id);
...
//	}
	...
	mysql_query("UPDATE fairs SET catmap='$catmap',divmap='$divmap' WHERE id='$fair_id'");
	echo "UPDATE fairs SET catmap='$catmap',divmap='$divmap' WHERE id='$fair_id'";
	echo mysql_error();
...
case 'additional_materials':
	...
	$q = mysql_query("SELECT award_source_fair_id,external_identifier FROM award_awards WHERE id='$award_awards_id'");
	$a = mysql_fetch_assoc($q);
	$q = mysql_query("SELECT * FROM fairs WHERE id='{$a['award_source_fair_id']}'");
	$fair = mysql_fetch_assoc($q);
...
case 'load':
	...
	$winners = get_winners($award_awards_id, $fair_id);
	$divs = projectdivisions_load();
	...
	if($fair['type'] != 'sfiab') {
		...
		list($server_cats, $server_divs, $catmap, $divmap) = load_server_cats_divs($fair_id);
		$division_disabled = false;
	}

?>
	<br /><br />
	<button onClick="popup_divmap(<?=$fair_id?>);return false;" <?=$division_disabled ? 'disabled="disabled' : ''?>
...
{
	var ids = "&id="+award_awards_id+"&fair_id="+fair_id;
...
function popup_divmap(fid)
{
	...
	/* Load dialog content */
	$("#popup_divmap").load("<?=$_SERVER['PHP_SELF']?>?action=catdiv_load&fair_id="+fair_id);
...
$q = mysql_query("SELECT fairs.id, fairs.name, fairs.type, COUNT(award_awards.id) as AWARD_COUNT FROM fairs 
			LEFT JOIN award_awards ON award_awards.award_source_fair_id=fairs.id
			WHERE award_awards.award_source_fair_id IS NOT NULL
...
$q = mysql_query("SELECT award_awards.id, award_awards.name AS awardname,
			  fairs.name as fairname, award_source_fair_id, 
			  fairs.type as fairtype, award_awards.external_additional_materials
			FROM award_awards 
			LEFT JOIN fairs ON fairs.id=award_awards.award_source_fair_id 
			WHERE award_awards.award_source_fair_id IS NOT NULL
				AND award_awards.year='{$config['FAIRYEAR']}'
...
<?
while($r=mysql_fetch_object($q)) {
	$count = count_winners($r->id, $r->award_source_fair_id);
?>	
...
<?
	if($count > 0) 
		$onclick = "popup_upload({$r->award_source_fair_id},{$r->id});return false;";
	else 
