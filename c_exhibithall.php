...
if($action == 'pn') {
	...
	foreach($projects as $p) {
		...
		$pn = "$c $n $d";
		$sort = sprintf("%04d", $p['floornumber']);

		TRACE("Project {$p['projects_id']} at loc {$p['floornumber']}: $pn\n");
		mysql_query("UPDATE projects SET projectnumber='$pn',projectsort='$sort' WHERE id='{$p['projects_id']}'");
	}
	TRACE("Done.\n");
	exit;
}
...
foreach($projects as $pid=>$p) {
	...
	TRACE("UPDATE projects SET floornumber='{$p['floornumber']}' WHERE id='$pid';\n");
	print(mysql_error());
